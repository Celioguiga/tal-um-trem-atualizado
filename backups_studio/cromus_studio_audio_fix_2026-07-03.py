...
def _sintaxe_para_ly_raw(sintaxe, compasso, compassos_por_linha=4, andamento=80, tonalidade="c \\major"):
    s = sintaxe
    s = s.replace('`', "'")
    s = re.sub(r'\(\s*CASA\s*(\d+)\s*\)', ',', s)
    for junk in ['||:', ':||', '||', '|', 'FIM', 'CASA1', 'CASA2', '[1', '[2']:
        s = s.replace(junk, ',')
    s = re.sub(r',,+', ',', s)
    s = s.strip(',').strip()
...
function preprocessSintaxe(s){
  // Espelha o preprocessamento do _sintaxe_para_ly_raw (Python)
  s = s.replace(/`/g, "'");
  s = s.replace(/\(\s*CASA\s*(\d+)\s*\)/g, ',');
  for(const junk of ['||:', ':||', '||', '|', 'FIM', 'CASA1', 'CASA2', '[1', '[2']){
    s = s.replaceAll(junk, ',');
  }
  s = s.replace(/,+/g, ',');
  s = s.replace(/^[,\s]+|[,\s]+$/g, '');
