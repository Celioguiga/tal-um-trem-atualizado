...
def _converter_tempo(grupo):
    ...
    tokens = _re.findall(r"[0-7]'*\*+|-\*+|[0-7]'*|-", g.replace(' ',''))
    notas = []
    for t in tokens:
        if '*' in t:
      ...
      <div class="pane" id="p-tab">
        <div style="padding:14px;display:flex;flex-direction:column;gap:12px;height:100%">
          <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            <span style="font-weight:600">Sintaxe:</span>
            <textarea id="tabSintaxe" rows="2" spellcheck="false"
              style="flex:1;font:14px var(--mono);padding:6px;background:var(--bg);
              border:1px solid var(--line);border-radius:6px;color:var(--ink);resize:vertical"
              placeholder="Cole trecho da Sintaxe Cromus (ex: 1, 2, 3, 4, 5)"></textarea>
          </div>
          <div style="display:flex;gap:8px;align-items:center">
            <label>Tonalidade:
              <select id="tabTonalidade" style="background:var(--bg);color:var(--ink);border:1px solid var(--line)">
            ...
            <button class="btn" onclick="gerarTab()">Gerar Tablatura</button>
          </div>
          <div id="tabOutput" style="flex:1;overflow:auto;min-height:200px">
            <div style="color:var(--dim);font-size:13px">Digite uma sintaxe e clique em Gerar Tablatura.</div>
          </div>
        </div>
      </div>
...
const GRAU_FORMA = {1:'círculo',2:'ogiva',3:'triângulo',4:'quadrado',5:'estrela',6:'hexágono',7:'casinha'};
const FORMA_SVG = {
...
function renderFretboard(notas, tonalidade){
  const W=600, H=260, marg=40, strings=6, casaH=22;
  const svg = [];
  svg.push(`<svg viewBox="0 0 ${W} ${H}" style="width:100%;max-width:660px;background:var(--bg);border-radius:8px">`);
  svg.push(`<defs><filter id="gshadow"><feDropShadow dx="0" dy="0.5" stdDeviation="0.5" flood-opacity=".4"/></filter></defs>`);
  // fundo
  ...
  for(let f=0; f<=12; f++){
    const y = y0 + f*stepY;
    svg.push(`<line x1="${marg}" y1="${y}" x2="${marg+(strings-1)*stepX}" y2="${y}" stroke="var(--line)" stroke-width="${f===0?2:.6}"/>`);
    if(f>0) svg.push(`<text x="${marg-8}" y="${y+4}" font-size="9" fill="var(--dim)" text-anchor="end">${f}</text>`);
  }
  // notas
  for(const n of notas){
    if(!n.posicao) continue;
    const p = n.posicao;
    const x = marg + (6-p.string)*stepX;
    const y = y0 + p.fret*stepY;
    const cor = CORES_RNFG[n.grau]||'#888';
    const forma = GRAU_FORMA[n.grau]||'circulo';
    const corClara = cor+'55';
    svg.push(`<circle cx="${x}" cy="${y}" r="11" fill="${corClara}" stroke="${cor}" stroke-width="2" filter="url(#gshadow)"/>`);
    // forma RNFG dentro
    if(FORMA_SVG[forma]){
      svg.push(FORMA_SVG[forma](x, y, 7, cor));
    }
    // tooltip
  ...
  // legenda
  let html = svg.join('');
  html += '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;font:12px var(--mono)">';
  for(let g=1; g<=7; g++){
    html += `<span style="display:inline-flex;align-items:center;gap:3px;padding:2px 6px;background:${CORES_RNFG[g]}33;border-radius:4px">
      <span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:${CORES_RNFG[g]}"></span>
      ${GRAU_NOME_ROM[g]} (${grau_forma_nome(g)})</span>`;
  }
  html += '</div>';
  // tab tradicional
  html += '<div style="margin-top:12px;font:12px var(--mono)"><strong>Tablatura (números):</strong><br>';
  const tab = [['','','','','',''], ['','','','','','']];
  // Actually let's just show a simple text version
  ...
  for(let s=5; s>=0; s--) html += tabStrs[s] + '<br>';
  html += '</div>';
  $('tabOutput').innerHTML = html;
}
function grau_forma_nome(g){ return GRAU_FORMA[g]||'—'; }
