...
function renderFretboard(notas, tonalidade){
  const W=600, H=290, marg=44, strings=6, stepY=18;
  const svg = [];
  ...
  for(let s=0; s<strings; s++){
    const x = marg + s*stepX;
    svg.push(`<text x="${x}" y="${y0-6}" font-size="9" font-weight="600" fill="var(--dim)" text-anchor="middle">${nomesCordas[s]}</text>`);
  }
  // agrupar notas por posição (corda+traste) para evitar sobreposição
  const posMap = {};
  for(const n of notas){
  ...
  for(const key in posMap){
    ...
    for(let i=0; i<count; i++){
      ...
      svg.push(`<circle cx="${x}" cy="${y}" r="9" fill="${cor}55" stroke="${cor}" stroke-width="1.8" filter="url(#gshadow)"/>`);
      if(FORMA_SVG[forma]){
        svg.push(FORMA_SVG[forma](x, y, 5.5, cor));
      }
  ...
  // legenda
  let html = svg.join('');
  html += '<div style="display:flex;flex-wrap:wrap;gap:4px;margin-top:6px;font:11px var(--mono)">';
  for(let g=1; g<=7; g++){
    html += `<span style="display:inline-flex;align-items:center;gap:2px;padding:1px 5px;background:${CORES_RNFG[g]}33;border-radius:3px">
      <span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:${CORES_RNFG[g]}"></span>
      ${GRAU_NOME_ROM[g]}</span>`;
  }
  html += '</div>';
  // tab tradicional
  html += '<div style="margin-top:8px;font:11px var(--mono);overflow-x:auto;white-space:pre;color:var(--dim)"><strong>Tablatura:</strong><br>';
  ...
  html += '</div>';
  // sequência de graus
  html += '<div style="margin-top:6px;font:11px var(--mono);color:var(--dim)">';
  html += '<strong>Graus:</strong> ';
  for(const n of notas){
    const cor = CORES_RNFG[n.grau]||'#888';
    html += `<span style="display:inline-block;padding:0 3px;color:${cor};font-weight:600">${n.grau}</span>`;
  }
  html += '</div>';
