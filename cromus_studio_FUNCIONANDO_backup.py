...
def _sintaxe_para_pipeline(sintaxe):
    import re as _re
    D={'w':'w','h':'m','q':'s','e':'c','s':'c','t':'c'}
    r=[]
    for t in _re.sub(r'[,|]',' ',sintaxe).split():
        m=_re.match(r'^([0-7])[#b]?(w|h|q|e|s|t)?',t)
        if m: r.append(m.group(1)+D.get(m.group(2) or 'q','s'))
    return ' '.join(r)

def gerar_arquivo_ly(sintaxe, modo, titulo, compasso):
    import tempfile, os
    fn, nome = _localizar_parser()
    notas_pipeline = _sintaxe_para_pipeline(sintaxe)
    cantiga = {
        "titulo": titulo, "compositor": "Synemusic", "compasso": compasso,
        "tonalidade": "c \\major", "andamento": 80, "compassos_por_linha": 4,
        "vozes": [{"notas": notas_pipeline}],
    }
...
<title>Cromus Studio · Synemusic</title>
...
<header>
  ...
  <h1>Cromus Studio <small>v2.0.1 · Synemusic</small></h1>
  <div class="hdr-right">
      ...
      <div class="pane" id="p-audio">
        <div class="acard">
          ...
          <div class="inst-w">Instrumento
            <select id="instSel">
              <option value="am">Piano (AMSynth)</option>
...
function parseSintaxeAudio(sintaxe){
  _events=[];
  const bpm=parseFloat($('bpmVal').value)||80, beat=60/bpm;
  const toks=sintaxe.replace(/,/g,' , ').split(/\s+/).filter(Boolean);
  let time=0;
  for(const tok of toks){
    ...
    const beats=DUR_BEATS_MAP[durK]*(dot?1.5:1);
    let note=GRAU_NOTE[grau]||null;
    if(note&&acc==='#') note=Tone.Frequency(note).transpose(1).toNote();
    if(note&&acc==='b') note=Tone.Frequency(note).transpose(-1).toNote();
    _events.push({note:grau===0?null:note, dur:DUR_TONE[durK]||'4n', time});
    time+=beats*beat;
  }
  _totalTime=time;
}
...
async function audioPlay(){
  ...
  const pi=setInterval(()=>{
    ...
    $('progFill').style.width=Math.min(100,(Tone.Transport.seconds/_totalTime)*100)+'%';
  },80);
  Tone.Transport.start();
  _playing=true;$('playBtn').textContent='⏸';
...
def main():
    ...
    print(f"🎼  Cromus Studio v2.0.1 → {url}  (Ctrl+C para sair)")
