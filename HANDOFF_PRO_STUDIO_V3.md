...
8. **Item 6 do roadmap — Áudio realista (violão nylon, amostras reais):** ✅ feito (2026-08-03). Trocado o sintetizador triangular (`Tone.PolySynth(Tone.Synth,...)`) por `Tone.Sampler` com amostras reais de violão nylon. Origem: SoundFont **"MS Basic.sf3"** (`share/sound/MS Basic.sf3` em github.com/musescore/MuseScore, MIT — cadeia completa Frank Wen→Michael Cowgill→S. Christian Collins, ver `CREDITS.md`). Guiga confirmou o timbre nylon (vs. aço) ouvindo os dois lado a lado. Extração: 21 notas (MIDI 36-96, passo de 3 semitons — cobre toda a faixa de escrita da Sintaxe Cromus, incluindo oitavas para cima/baixo), programa General MIDI 24, renderizadas via `fluidsynth` (já instalado via Homebrew) a partir de MIDI mínimos gerados sem lib externa (`mido`/`pretty_midi` não disponíveis — script cru de ~20 linhas monta o `.mid` byte a byte), cortadas/comprimidas via `ffmpeg` (mono, 22050Hz, MP3 96kbps, ~30KB/nota) e embutidas em base64 em `src/guitar_samples.js` (novo arquivo, ~791KB — `pro_studio.html` foi de ~977KB pra ~1.77MB). `build.py` ganhou um 3º `<script>` só pra esse arquivo (tamanho). `ensureSynth()` agora é assíncrono (`Tone.Sampler` carrega os buffers, mesmo vindo de `data:` URI — sem rede, mas ainda precisa decodificar) — retorna `Tone.loaded()`, e os 2 call sites que chamavam sem `await` (clique-para-ouvir, `play()`) ganharam `await`. `exportWav()`: o callback de `Tone.Offline` virou `async` e faz `await Tone.loaded()` antes de agendar qualquer `triggerAttackRelease` — confirmado que `Tone.Offline` aceita callback assíncrono nesta versão (14.8.49). Validado: `synth.loaded===true` após `ensureSynth()`; playback ao vivo (1 e 2 vozes) sem erro de console; WAV exportado (1 e 2 vozes) com pico de amplitude real (não silencioso) via leitura direta dos samples int16 do buffer. `CREDITS.md` novo, com o texto MIT completo (exigência da licença — avisos de copyright precisam acompanhar qualquer trabalho derivado).
